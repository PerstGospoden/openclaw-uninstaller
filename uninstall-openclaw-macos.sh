...
usage() {
  cat <<'EOF'
Usage: ./uninstall-openclaw-macos.sh [--dry-run|-n] [--help|-h]
EOF
}
...
if [ "$OFFICIAL_OK" -eq 1 ]; then
  ...
  if cmd_exists brew; then
    ...
    info "brew services stop attempted"
  fi
else
...
if [ "$OFFICIAL_OK" -ne 1 ]; then
  if cmd_exists brew; then
    ...
    info "homebrew uninstall attempted"
  fi
...
remove_if_exists "$HOME/Library/Application Support/openclaw"
remove_if_exists "$HOME/Library/Caches/openclaw"
remove_if_exists "$HOME/Library/Logs/openclaw"
